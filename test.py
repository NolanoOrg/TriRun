...
def gen_quant2(m, n, groupsize=-1):
    ...
    layer = trirun.Layer(256, 256, groupsize=groupsize)
    if groupsize == -1:
        groupsize = m
    layer.k = m
    layer.n = n
...
class Test(unittest.TestCase):
    ...
    def run_problem(self, m, n, k, thread_k, thread_n, groupsize=-1):
        ...
        trirun.mul(A, B, C, s, workspace, thread_k, thread_n, -1)
    ...
    def test_errors(self):
        ...
        err = False
        try:
            trirun.mul(A, B, C, s, workspace, 128, 128, -1)
        except:
            err = True 
        self.assertTrue(err)
        err = False
        try:
            trirun.mul(A, B, C, s, workspace, 256, 256, -1)
        except:
            err = True 
        self.assertTrue(err)
